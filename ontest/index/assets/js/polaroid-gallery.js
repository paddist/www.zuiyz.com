    ...
    function setGallery(arr) {
        ...
        for (i = 0; i < arr.length; i++) {
            out += '<figure id="' + i + '">' +
                ...
                '<div style="display:none;"><span class="min">' + arr[i].min + '</span><span class="max">' + arr[i].max + '</span></div>' +
                '</figure>';
        ...
        $('#gallery').children().each(function(){
            ...
            var floderArr = src.split('/');
            var floder = floderArr[floderArr.length - 1].substr(0, floderArr[floderArr.length - 1].indexOf('.'));
            var title = encodeURIComponent($(this).find('figcaption').html());
            var min = $(this).find('.min').html();
            var max = $(this).find('.max').html();
            $(this).click(function(){
                window.open('//zuiyz.com/detail/index.html?title=' + title + '&floder=' + floder + '&min=' + min + '&max=' + max);
            });
        });
    }
